...
# `git` and `make` are required by FetchContent (clones) and by the
# liburing autotools fallback (configure + make), so every distro path
# installs them — without them the floor build fails before the first
# subsystem source is touched.
case "$ID:$VER" in
  ubuntu:24.04)
    apt-get update
    apt-get install -y \
  ...
  ubuntu:22.04)
    ...
    # --batch --yes makes the dearmor idempotent on rerun.
    wget -qO - https://apt.kitware.com/keys/kitware-archive-latest.asc \
      | gpg --batch --yes --dearmor -o /usr/share/keyrings/kitware.gpg
  ...
  fedora:*)
    dnf install -y \
      ...
      kernel-headers git make ca-certificates \
      valgrind cppcheck gdb
    ;;
  rhel:9*|almalinux:9*|rocky:9*)
    # RHEL/Alma/Rocky 9 report VERSION_ID like "9.4"; match the family
    # with a glob, not exact "9". Stock cmake is 3.20; need pip for 3.25+.
    # gcc-toolset-12 lives at /opt/rh/gcc-toolset-12 and is NOT on PATH
    # by default — see follow-up note printed at end of script.
    dnf install -y python3-pip gcc-toolset-12 ninja-build pkg-config \
      kernel-headers git make ca-certificates
    pip3 install --no-cache-dir 'cmake>=3.25'
    ;;
  *)
    echo "setup.sh: unsupported distro: $ID:$VER" >&2
    echo "  Supported: ubuntu 22.04/24.04, debian 12, fedora 38+, rhel/alma/rocky 9.x" >&2
    exit 1
    ;;
esac
...
case "$ID:$VER" in
  rhel:9*|almalinux:9*|rocky:9*)
    echo "  RHEL family: gcc-toolset-12 is installed but NOT on PATH by default."
    echo "  Run: scl enable gcc-toolset-12 -- bash    (or source /opt/rh/gcc-toolset-12/enable)"
    echo "  Then: cmake --preset default && cmake --build --preset default && ctest --preset default"
    ;;
  *)
