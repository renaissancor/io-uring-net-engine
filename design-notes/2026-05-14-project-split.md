...
│   ├── doc/                        # cross-cutting design
│   ├── doc/                        # per-source-file specs
│   └── (installs)
...
Everything else — `src/_placeholder.cpp`, `tests/`, `doc/`, build
...
being specified now (in the sibling's `doc/`+`doc/`) matters more
