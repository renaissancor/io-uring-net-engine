...
- Doc cleanup landed: stale coroutine references removed from `doc/*.md` (commit `738d2bb`).
...
- `doc/runtime/threading_model.md` — "two-tier reactor" framing
...
in `doc/*.md` never were, leaving misleading claims that the project
...
allowed to use `std::` freely. `doc/04-coding-style.md` even
...
`doc/04-coding-style.md` and the memory entry both updated in this
commit.
