...
- **Trap mechanism is `int 3` / SIGTRAP** (not `__builtin_trap` / SIGILL). Cross-compiler via `__builtin_debugtrap` on Clang, inline asm on GCC. Cross-arch via `int3` (x86) / `brk #0xf000` (aarch64). Reference: `wiki/check.md`, `src/check.h`.
- **`LNX_CHECK` vs `LNX_DCHECK` split**: cold-path primitives (`lnx::thread`) use always-on `LNX_CHECK`; hot-path primitives (`lnx::mutex`) use debug-only `LNX_DCHECK`. Matches Folly/abseil/Chromium. Reference: `wiki/check.md`.
...
1. **`lnx::lock_free_stack<T>`** — Treiber stack with tagged-head ABA protection. Design already in `wiki/sync/lock_free_stack.md`. Small, well-spec'd. Uses primitives that just landed (`lnx::atomic*`, `lnx::cache_aligned`).
...
The wiki page is currently named `wiki/sync/lock_free_stack.md` and frames the structure as "memory pool free list." After yesterday's discussion, this is misleading — the structure is the object-pool primitive. When implementing, also:

- Rename `wiki/sync/lock_free_stack.md` purpose section: "primitive behind object pools, not the byte-allocating memory pool."
- Write `wiki/sync/object_pool.md` as the user-facing wrapper doc.
- Spec `wiki/memory/memory_pool.md` separately for the TLS-first byte allocator (defer writing until step 4 is imminent).
...
- `wiki/memory/memory_pool.md` — TLS-first byte allocator design. Lands with step 4.
- `wiki/sync/object_pool.md` — generic object pool on top of `lock_free_stack`. Lands with step 2.
- `wiki/diagnostic/ebpf_observability.md` — uprobes / tracepoints / bpftrace snippets for this project. Lands when the server is real enough to run under load.
- `wiki/runtime/crash_handler.h` doc — sigaction + backtrace for SIGTRAP/SIGSEGV. Lands when there's a real reactor process to instrument.

## Quick-start for next session

1. `git status` → expect clean.
2. `make test` → expect 73/73.
3. Read `wiki/sync/lock_free_stack.md` — the design is already there, just needs implementation.
