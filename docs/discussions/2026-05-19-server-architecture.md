...
- Cross-thread `shared_ptr` deleter concern from `wiki/memory/object_pool.md` evaporates — Session is single-thread-owned.
...
The wiki specs in `wiki/memory/*.md`, `wiki/runtime/job_queue.md`, `wiki/network/io_uring_reactor.md`, and `wiki/network/session.md` are **ahead of the actual design intent** on the TLS pool axis and owe updates to reflect channel-per-thread, shared-nothing-within-a-channel.
...
7. **`session_handle.md` spec missing** — Referenced in `docs/00-overview.md:25` and `README.md:117` but the spec file was never written.
