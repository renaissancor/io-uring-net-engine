...
the no-STL Linux policy (see `doc/04-coding-style.md`) plus the
...
project-wide `LNX_CHECK` philosophy (see `doc/check.md`). There is no
...
- **Address stability is the load-bearing guarantee.** Because storage is
  ...
  `doc/runtime/threading_model.md` and the `.omc` handle/engine split
  design log.
...
3. **Shared use by SoA session/room storage.** Sessions and rooms also want
   ...
   `doc/network/session.md`). Decide whether they reuse `static_vector<T,
