...
worker_ctl::worker_ctl(i32 id, const config& cfg) noexcept
    : base(id, ""),   // base name set below
      _cfg(cfg)
{
    // Single decimal digit (0 .. k_worker_count-1) keeps every name 8 chars
    ...
    LNX_CHECK(id >= 0 && id < roster::k_worker_count);
...
void worker_ctl::start() noexcept {
    ...
    base._thread = lnx::thread{&worker_ctl::entry, this};
}

void* worker_ctl::entry(void* self) noexcept {
    auto* h = static_cast<worker_ctl*>(self);

    // (1) Install role token FIRST — gates every TLS singleton that
    //     follows. Without this, worker_engine::instance()'s LNX_CHECK
    //     traps.
    ...
    worker_engine& eng = worker_engine::instance();
    eng.attach(h);
